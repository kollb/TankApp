...
> Stand: 20.09.2026 · App-Version 0.59.1. Abgleich von
...
| Bereich | Vor B5 (10.09.) | Heute (0.43.2) |
|---|---|---|
...
[Optimierungs-Befunds](archiv/OPTIMIERUNGS-BEFUND-2026-09-18.md#batch-8--p3--schliff) sind
...
[Befund](archiv/OPTIMIERUNGS-BEFUND-2026-09-18.md#batch-7--p2--betrieb-rest). Der Batch ändert
...
[Befund](archiv/OPTIMIERUNGS-BEFUND-2026-09-18.md#batch-6--p2--anzeige-und-alltag). Bewusst offen
...
[Befund](archiv/OPTIMIERUNGS-BEFUND-2026-09-18.md#batch-6--p2--anzeige-und-alltag). Zwei Punkte
...
**Maßnahme (d) — das Aufteilen der Veröffentlichung je Kraftstoff/Station —
ist seit 0.49.0 umgesetzt.** Auslöser war der vereinbarte Trigger: Mit der
zweiten Stadt wuchs das Polling-Set von 11 auf 20 Stationen und die kompakte,
gerundete Monolith-Datei auf 13,5 MB — über das Leselimit, „keine Prognose“
überall. Jetzt liegt jede Stations-Prognose in einer eigenen Datei unter
`runtime/engine/forecasts/`, `current.json` ist ein kleiner Index mit Zeigern
(`layout: "split-forecast-files"`), und `publication()` fügt beides zur
gewohnten Bundle-Form zusammen, sodass die Leser unverändert bleiben. Die
Abwägung von damals steht in der Backlog-Tabelle unten (dort jetzt als
erledigt markiert).

### 16.09.2026 — Version 0.43.2: Mobil-Robustheit gemessen statt behauptet

Der Befund war eine Nutzerwahrnehmung („verschiedene Dinge die scrollen müssen
oder aus dem Bild ragen“) — also nichts, was ein Unit-Test sieht. Deshalb steht
die Zusage jetzt als Messung in `web/e2e/mobile.spec.ts`: je Zustand werden
`scrollWidth`/`clientWidth` des Dokuments, jedes Elements, das über den
Viewport hinausragt, und jeder Kasten, der breiter malt als er ist, erhoben;
die Fundstellen stehen als `[mobil] <Zustand>: {…}` im Log. Gemessen werden
alle sechs Bereiche, die vier Ich-Unterseiten (die Belege legt der Test vorher
über die echte API an — sonst bliebe die längste Liste ungeprüft), die sechs
Labor-Abschnitte und jeder Dialog eines Bereichs. Eine letzte Prüfung hält die
Liste der **bewusst** schiebbaren Kästen fest: heute genau die Heatmap-Matrix
(24 Stunden × Wochentage lassen sich nicht stapeln; der Text darüber sagt es
mobil an, MICROCOPY §4c). Drei Funde der ersten Messung sind behoben — die
Steuerzeile der Kopfzeile (748 px in 209 px Fenster, jetzt `flex-wrap`,
Wortmarke mobil aus, `sm:sticky`), „Stationen“ in der unteren Navigation (2 px
über der Zelle, jetzt `tracking-tighter` + `truncate`) und die `<pre>`-Blöcke
in System/Labor (413 px in 308 px, jetzt mobil umbrechend). Aus demselben
Befund ebenfalls mobil entzerrt: die Belegliste (560 px Mindestbreite →
Karten, ab `sm` die Tabelle, beide aus `web/src/fills.ts`), die
Preis-Zwillings-Tabelle in System und die Entscheidungsschwellen in Ich.

**Und ein echter Fehler, den erst die Beleg-Prüfung sichtbar machte:**
`/?tab=ich` direkt geöffnet zeigte „Noch keine Belege“, auch wenn der Ledger
gefüllt war — der Verlauf kam als Teil der Overview-Antwort, und die holt nur
Jetzt/Stationen/Woche (der geteilte Link aus UI-NEUENTWURF §13 führte damit
auf eine leere Liste). Auf „Ich“ liest die Liste jetzt selbst
`GET /api/v1/fills`; `e2e/app.spec.ts` hält den Fall mit einer Attrappe fest.
...
Befunde aus PR #125 sind umgesetzt. **V3** ersetzt die acht gestapelten
Banner der Root durch ein Mitteilungs-Register mit Rang
(`components/Notices.tsx` `reduceNotices` + `components/NoticesView.tsx`):
Störung > Zustand > Hinweis > Erfolg, höchste Stufe gewinnt, Gleichrangige
werden aneinandergereiht, ein Block über dem Inhalt, Dauer 6 s (Störungen
bleiben). **V4** ersetzt die Symbolsprache durch Worte — Tagebuch
„richtig/daneben/unentschieden“, Chips „Jetzt tanken/Warten/Woanders
tanken“, Filter „nur offene/alle Stationen“, Due-Knöpfe ohne `✓/✎/✕`, ein
dekoratives lucide-Häkchen im Einrichtungs-Assistenten (`aria-hidden`).
**V5** zieht die Einheiten-/Zeitraumformen in `data.ts` zusammen
(`euroPerHour`, `kilometersPerHour(Speech)`, `timeSpanLabel`) und schreibt
das Queue-Alter über `ageWord`. Ratchets: `microcopy.test.ts` Regel 11
(Symbole) + 12 (Einheiten/Abkürzungen), `components/Notices.test.ts` neu,
`data.test.ts` hält die Formatter. Keine neuen Endpunkte, keine
Engine-Änderung, keine neuen Lücken — Konzept-Abdeckung unverändert.
Damit ist der GUI-Text-Befund (PR #125) vollständig umgesetzt.
...
| Punkt | Umsetzung | Prüfung |
|---|---|---|
| Alltag in einer Anfrage | `GET /api/v1/overview` → `DataApi.overview()`: `decide`, `fills`, `stats/summary`, due-Episoden und die 24-h-Tageskurve in **einer** Antwort — dieselben Bausteine wie die Einzelrouten, exakt deren Einzel-Formen. Unbekannte Station entlädt nur die Tageskurve, `decide` wählt selbst. `route/evaluate` bleibt bewusst ein eigener Poll (Follow-up) | B7-Rest, `tests/test_app.py` (8 Fälle) |
| Refresh revalidiert statt neu zu laden | `data_version()` (reine Datei-Stats: Collector-Heartbeat, Engine-/Selektions-Artefakte, Feedback-Store, Polling-Set + 60-s-Uhrzeit-Fenster für das uhrzeitabhängige „due“-Status/`decide`-Fenster) → `ETag` auf `/overview`; `If-None-Match` bei gleichem Datenstand → **304 ohne Body, ohne Compute** + Antwort-Cache je (Datenstand, Parameter) für Anfragen ohne If-None-Match. `useResource` schickt das ETag mit und behält bei 304 die Anzeige. Begründung: Der Token-Bucket lässt höchstens 1 Preis-Poll/300 s zu — die meisten Refreshes rechnen sonst dieselbe Antwort neu | B7, `tests/test_app.py` + `data-resource.test.tsx` (Protokoll-Test) |
...
| 0.4 | Kalibrierungs-Gate (Intervall-Obergrenze < Referenzen, n ≥ 100) |fertig als hartes Gate (O6, 0.45.0: Block-Bootstrap über Tagesblöcke gegen Basis- und Klima-Referenz statt Punkt-Brier gegen 0,25); offen bis echte Daten (M7) |
...
| **Preis-Ticker (§12 P2)** | entschieden | Der **Alarm**-Push ist seit 0.15.0 drin (ntfy, `severity: error`), und seit 0.46.0 meldet sich das **empfohlene Fenster** (O29: offen/geändert/verstrichen, je Episode eine Meldung, Ruhezeit 22–7 Uhr; die Versand-Regel ist als Push-Modus `public`/`lan` entschieden, O42 → [BETRIEB.md](BETRIEB.md#alarm-zustellung-über-ntfy-b4)). Offen bleibt allein der Preis-Ticker „Jetzt 4 ct unter Tagesmedian“ — eine Meldung ohne Empfehlung, und für die gilt §11 des GUI-Neuentwurfs weiterhin (keine Preis-Erinnerungen). **18.09.2026 entschieden:** bleibt so — was der Nutzer braucht, ist die Empfehlung, nicht der Kursalarm. |
| **Top-3-Fenster-Trefferquote (§6)** | entschieden | Die Engine veröffentlicht je Tag eine Prognosestunde; drei Kandidatenfenster wären geraten. **18.09.2026 entschieden:** kein Arbeitspunkt mehr — die Quote über das eine veröffentlichte Fenster ist ehrlich und reicht; drei Fenster wären eine Kennzahl über erfundene Kandidaten. |
...
| **Ensemble-Gewichte aus dem Validierungsfenster statt aus dem Backtest (A10)** | entschieden | Die Gewichte ∝ 1/MASE entstehen aus der Eine-Schritt-Prognose (5 min) auf den letzten 14 Trainingstagen. Dort trennen sich die Modellkerne kaum: 0,51 / 0,49, obwohl der 72-Stunden-Fehler 2,53 vs. 1,86 ct/L sagt. Grund: auf 5 Minuten dominiert der AR(2)-Nachlauf, den beide Modelle gemeinsam haben; der Unterschied der Tagesform wirkt erst auf Stunden. Richtig wäre die Gewichtung aus dem Rolling-Origin-Backtest (Fehler über 24/72/168 h — genau die Horizonte, die die App ausgibt). Das ist mehr als ein Parameter: der Backtest müsste beide Kerne je Fold bewerten, und die Gewichte müssten je Horizont getrennt geführt werden. Bis dahin ist das Ensemble das, was die Messung oben zeigt: deutlich besser als der alte Hauptpfad, etwas schlechter als das Zweitmodell allein — und beides ist ausgewiesen statt behauptet. **O6-Querverweis (0.45.0):** Das Gate-Muster (Intervall-Obergrenze gegen Referenzen statt Punkt gegen Schwelle) ist die Vorlage für den Rebuild — Gewichte dann ebenfalls mit Unsicherheit statt als Punktzahl. **18.09.2026 entschieden:** Das Ensemble ist gemessen besser als der alte Hauptpfad und die Abweichung ist ausgewiesen — ein Rebuild des Backtests für eine zweite Nachkommastelle der Gewichte steht in keinem Verhältnis. |
...
| **Mobil: die Kopfzeile sind vier Steuerzeilen (197 px bei 390 px)** | Arbeit | Gemessen im Sandkasten: Stadt (139 px), Kraftstoff (165 px), Profil (208 px), dazu Alarm/Teilen/Aktualisieren — vier Zeilen über dem Inhalt, bevor „Jetzt“ beginnt (253 px bis zur Überschrift bei 2 118 px Gesamtlänge). **Nicht** angefasst, weil jede weitere Verdichtung Steuerungen aus der Zeile auslagert (Blattmenü, Symbole statt Beschriftungen) und damit §5.1 „Gemeinsame Kopfzeile: ☰? TankApp · Stadt · E10 [●] [⌘K]“ ändert — das ist eine Konzeptentscheidung, keine CSS-Aufgabe. Zwei Wege wären zu prüfen: (a) Stadt + Kraftstoff + Profil hinter ein Blatt „Einstellungen“ (ein Knopf statt drei), (b) Kopfzeile auf Höhe scrollen lassen und nur die Alarm-Pille anheften. **18.09.2026 als nicht nötig geschlossen:** 0.53.0 hat die Verdichtung gebracht, die den Anlass hatte (2 530 px → 2 118 px, Zusage in `web/e2e/mobile.spec.ts`). Die restlichen 197 px sind Steuerzeilen; sie hinter ein Blattmenü zu räumen kostet im Alltag einen Griff mehr für einen Bildschirm, der passt ([TODO.md](../TODO.md#geschlossen-als-nicht-nötig-18092026)). |
| **Zweites automatisches Backup-Ziel (O33-Rest, B25)** | Arbeit | Seit 0.47.0 meldet die App die Backup-Alterung selbst (`backup_stale` ab 36 h ohne neues `tankapp-runtime-*.tar.gz`, dazu `no_backup`/`directory_missing`), und `ops/nas/backup.sh` behält 14 Tages- plus 6 Monatsstände — ein langsam zerstörender Fehler hat damit einen Stand, zu dem man zurück kann. Offen bleibt Lage (c) des Befunds: Das Ziel liegt auf demselben Gerät wie die Daten, ein NAS-Ausfall nimmt Daten **und** Sicherung mit. Die unersetzbaren Bestände (Tank-Bilanz, Polling-Set mit den Anker-Koordinaten) haben eine **manuelle** Zweitkopie als Entscheidung ([BETRIEB.md](BETRIEB.md#nas-laufzeitdaten-runtime-backup)), aber kein zweites automatisches Ziel. **18.09.2026 als nicht nötig geschlossen:** Alarm bei Alterung, 14 Tages- plus 6 Monatsstände und die dokumentierte manuelle Zweitkopie decken die Lagen ab, die im Haushalt vorkommen. Ein zweites automatisches Ziel bräuchte ein zweites Gerät, das es nicht gibt — Automatik gegen fehlende Hardware wäre ein Alarm ohne Adressat ([TODO.md](../TODO.md#geschlossen-als-nicht-nötig-18092026)). |
...
Am 18.09.2026 trug keine Zeile mehr die Marke „Arbeit“: Mit dem Abschluss des
Optimierungs-Befunds (0.54.0) hatte der Betreiber die vier verbliebenen
Arbeitspunkte (B25, C13, B22, C12) und die offenen Betriebsaufgaben als *nicht
nötig* geschlossen — die Gründe stehen je Zeile oben und gesammelt in
[TODO.md](../TODO.md#geschlossen-als-nicht-nötig-18092026).

**Seit dem 19.09.2026 trägt wieder eine Zeile die Marke „Arbeit“:** der
Regime-Wechsel durch Tankrabatt (01.10.2026) und Spritpreisdeckel (01.01.2027)
— ein Punkt, der aus dem Gesetz kommt und nicht aus dem Betrieb, mit Fristen,
die der Kalender setzt. Die Kategorie hat also ihren Zweck erfüllt.
