...
| PIT-Kalibrierung (B2, 0.57.0) | `TANKAPP_CALIBRATION=1` (Default) aktiviert nur zuvor zeitlich getrennt akzeptierte, stations-/Modell-/Draw-Modus-genaue PIT-Kurven für neue 24-h-Bootstrap-Pfade. `=0` ist die dokumentierte A/B-Gegenmessung: Kandidaten und Kennzahlen laufen weiter, die Veröffentlichung bleibt technisch unkalibriert. Jeder neue Advice-Snapshot markiert seine damalige Gruppe (`raw`/`pit_24h`); das Labor zeigt den allzeitlichen Ledger-Brier getrennt, Altbestand ehrlich als `unknown`. Kandidat/aktive Hülle stehen je Station als `calibration_candidate`/`calibration`; M7 bleibt ein separates Ledger-Gate. Nach Wechsel der Variable einen vollständigen Modell-Lauf abwarten — der Kandidat wird absichtlich erst im Folgelauf angewandt. |
