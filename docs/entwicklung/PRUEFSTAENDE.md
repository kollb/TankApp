# Prüfstände der technischen Referenzen

> Stand: 21.09.2026 · App-Version 0.62.0

## Inhaltsverzeichnis

- [Nicht gegen die aktuelle Version geprüft](#nicht-gegen-die-aktuelle-version-geprüft)
- [Pflege](#pflege)

## Nicht gegen die aktuelle Version geprüft

Diese Dokumente behalten ihre ursprüngliche fachliche Stand-Zeile. Umzug,
Linkreparatur oder redaktionelle Neuordnung sind keine Vollabnahme gegen
0.62.0. Einzelne neuere Abschnitte können zusätzlich gekennzeichnet sein.

| Dokument | Letzter angegebener Prüfstand |
|---|---|
| [MICROCOPY.md](../produkt/MICROCOPY.md) | 0.61.0 |
| [RP2.md](../betrieb/RP2.md) | 0.61.0 |
| [UI](../produkt/UI.md) | 0.60.0 |
| [README.md](../../README.md) | 0.59.1 |
| [CONTRIBUTING.md](../../CONTRIBUTING.md) | 0.59.1 |
| [ADR-Index](../adr/README.md) | 0.59.1 |
| [DOKUMENTATION.md](DOKUMENTATION.md) | 0.59.1 |
| [KONZEPT.md](../produkt/KONZEPT.md) | 0.59.1 |
| [REGIME.md](../planung/REGIME.md) | 0.59.1 |
| [ARCHITEKTUR.md](../architektur/ARCHITEKTUR.md) | 0.38.0 |
| [BETRIEB.md](../betrieb/BETRIEB.md) | 0.53.0 |
| [INSTALL.md](../betrieb/INSTALL.md) | 0.38.0 |
| [STATIONEN-TAUSCH.md](../betrieb/STATIONEN-TAUSCH.md) | 0.38.0 |
| [QUALITAET.md](QUALITAET.md) | 0.53.0 |
| [ANALYSE.md](../referenz/ANALYSE.md) | 0.38.0 |
| [API.md](../referenz/API.md) | 0.52.0 |
| [DATENWERKZEUGE.md](../referenz/DATENWERKZEUGE.md) | 0.55.2 |
| [ENGINE.md](../referenz/ENGINE.md) | 0.11.0 |

## Pflege

Bei fachlicher Durchsicht Stand-Zeile und diese Tabelle gemeinsam aktualisieren.
`tests/test_ledger_drift.py` prüft beide Richtungen. Archiv und Release-Historie
sind bewusst historische Quellen und gehören nicht in diese Rückstandsliste.
