...
| Gemessenes Modell (B0, seit 0.56.0; Vertrag seit 0.70.0) | Der Backtest misst per Default `harmonic_ar2` mit **unabhängiger** Ziehung (Stand vor A10/A11). Seit 0.56.0 stehen Mess- und Veröffentlichungsmodell nebeneinander (`backtest_model_kind` neben `model_kind`), dazu PIT-Paare je Station/Horizont und Regime-Marker im Bericht ([ENGINE.md](ENGINE.md#messgrundlagen-b0-seit-0560)). **Normativ ist seit 0.70.0 die Vertragsmatrix** (`app/model_contracts.py`): `decision_release=true` trägt im Produktivbetrieb nur `profile_ar2` (gemeinsame Ziehung, Day-Pair); `harmonic_ar2` und `ensemble` sind Forschungs- und Vergleichspfade ohne Entscheidungsfreigabe ([ENGINE.md](ENGINE.md#modell-vertragsmatrix-m1-seit-0700)). Umschalten des Messmodells ist B3 ([LUECKEN.md](../planung/LUECKEN.md#ausstehender-betriebsnachweis)) |
