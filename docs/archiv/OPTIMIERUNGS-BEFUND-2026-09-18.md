...
> **Archiviert am 18.09.2026 · App-Version 0.54.0.** Der Befund ist **vollständig abgearbeitet**: Batch 1–8 (O1–O44) sind mit 0.44.0–0.54.0 umgesetzt oder als Dauerzustand benannt, jeder Befund trägt seinen Umsetzungsvermerk. Das Dokument bleibt als Protokoll lesbar — Zeilenangaben und Messwerte stehen gegen 0.43.2 und werden nicht nachgeführt. Was heute gilt: [LUECKEN.md](../planung/LUECKEN.md) (Konzept ↔ Stand), [TODO.md](../planung/TODO.md) (Arbeitsliste), [CHANGELOG.md](../releases/CHANGELOG.md) (Nachweise je Version).
>
...
> werden die offenen Aufgaben A–H aus [TODO.md](../planung/TODO.md), die
...
> umgesetzt** ([LUECKEN.md](../planung/LUECKEN.md#implementierter-stand)):
...
> [TODO.md](../planung/TODO.md) (B25, C13, B22, C12).
...
[LUECKEN.md](../planung/LUECKEN.md) bereits mit denselben Zahlen dokumentiert und wird
hier nicht neu verkauft — es taucht nur als Querverweis in O6 auf, weil die
Brier-Schwelle dieselbe Schwäche erbt.

Seit dem ersten Durchgang ist **0.43.2** erschienen ([CHANGELOG.md](../releases/CHANGELOG.md)):
...
([MICROCOPY.md §4b](../produkt/MICROCOPY.md#4b-bereich-jetzt-feste-muster)).
...
existiert nicht. [LUECKEN.md](../planung/LUECKEN.md) Zeile 520 beschreibt das Gate so,
...
[KONZEPT.md](../produkt/KONZEPT.md)/[LUECKEN.md](../planung/LUECKEN.md) als M7-Kriterium.
...
[LUECKEN.md](../planung/LUECKEN.md) Zeile 516 bereits als diskriminanzarm dokumentiert
...
ein fehlendes Feld. [UI.md](../produkt/UI.md) Zeilen 588–597
spezifizieren den Hauspreis-Vergleich, [LUECKEN.md](../planung/LUECKEN.md) Zeilen 476
...
[ANALYSE.md](../referenz/ANALYSE.md) Zeile 135 nennt die echte Konfiguration:
...
[QUALITAET.md](../entwicklung/QUALITAET.md) nachziehen, damit das Budget nicht nur im
Browser steht.
...
`engine/storage.py:41–43`), aber [STATIONEN-TAUSCH.md](../betrieb/STATIONEN-TAUSCH.md)
...
vorgesehen ([TODO.md](../planung/TODO.md), Rahmenbedingungen). Befunde O29–O32
...
[LUECKEN.md](../planung/LUECKEN.md) Zeile 510 führt Preis-Push als bewusst offen
...
[MICROCOPY.md §4f](../produkt/MICROCOPY.md#4f-push-texte-alarme-und-fenster-meldungen).
...
| Externe Abhängigkeiten und Ausfall | **Kein eigener Befund.** [BETRIEB.md](../betrieb/BETRIEB.md) deckt Collector-, Uploader-, GLIBC- und Preislücken-Fälle breit ab, die Alarm-Codes sind benannt, OSRM ist optional und selbst gehostet (`app/data.py:61–68`) |
...
gesichert (`docs/betrieb/BETRIEB.md:680–698`), die Pi-Dateien laut Tabelle
...
[BETRIEB.md](../betrieb/BETRIEB.md#nas-laufzeitdaten-runtime-backup): ein Ziel auf dem
...
offen und steht im [Todo](../planung/TODO.md). Nachweis:
`tests/test_o33_backup_stale.py` (9 Fälle).

### O34 — Historie und Archiv haben keine Aufbewahrungsregel

**Beleg.** Der InfluxDB-Cron in `docs/betrieb/BETRIEB.md:684–688` schreibt
...
`docs/betrieb/BETRIEB.md:285–288`) kommt in keiner Sicherung vor (`grep archive`
...
**Wirkung.** `docs/referenz/ANALYSE.md:135` begründet B = 2000 statistisch
...
Parse-Zeit-Feld. [QUALITAET.md](../entwicklung/QUALITAET.md) führt Budgets für Lighthouse
...
[BETRIEB.md](../betrieb/BETRIEB.md) Zeilen 248–251.
...
[BETRIEB.md](../betrieb/BETRIEB.md) ausdrücklich benennen (was lesbar ist, für wen,
...
Beispiel-Einrichtung in [BETRIEB.md](../betrieb/BETRIEB.md#alarm-zustellung-über-ntfy-b4)
...
[MICROCOPY.md §4f](../produkt/MICROCOPY.md#4f-push-texte-alarme-und-fenster-meldungen)
und in [BETRIEB.md](../betrieb/BETRIEB.md#alarm-zustellung-über-ntfy-b4); der gewählte
...
Pfade sind bytegleich. Die Zitate aus `web/src/` und `docs/planung/LUECKEN.md` sind
...
| Ensemble-Gewichte | In-Sample-MASE ergibt rund 52/48 (0,173/0,185); echtes Out-of-Sample: Zweitmodell 1,64 ct gegen Hauptmodell 3,6 ct | `engine/models.py` auf synthetischen Daten — **bekannt**, in [LUECKEN.md](../planung/LUECKEN.md) Zeile 516 dokumentiert |
...
- **`ops/nas/backup.sh`** und [BETRIEB.md](../betrieb/BETRIEB.md) Zeilen 649–727 sichern
...
| `GET /api/v1/fills` zeigt `tanked_at`, `clock_hour`, `clock_hour_source` | erfüllt (API-Test + [API.md](../referenz/API.md#fills-b4-belege)) |
...
begründet in [LUECKEN.md](../planung/LUECKEN.md#ausstehender-betriebsnachweis).
...
| [O42](#o42--der-push-grundsatz-kollidiert-mit-dem-preis-push) Push-Grundsatz (P2, vorgezogen) | S | Entscheidung steht in `notify.py`-Docstring und [BETRIEB.md](../betrieb/BETRIEB.md); Test prüft den Server-Payload gegen die gewählte Regel (öffentlich: keine Preise, keine Stationen; selbst gehostet: Preis und Station erlaubt, keine Koordinaten, keine Pfade) |
...
| [O33](#o33--die-backup-alterung-bleibt-unsichtbar) Backup-Alarm | S | Test legt ein 40 Stunden altes `tankapp-runtime-*.tar.gz` an und erwartet `backup_stale` (warn); mit frischem Tar bleibt der Alarm aus; zweites Backup-Ziel und Aufbewahrungsregel stehen in [BETRIEB.md](../betrieb/BETRIEB.md) |
...
[BETRIEB.md](../betrieb/BETRIEB.md#nas-laufzeitdaten-runtime-backup) und im
[Todo](../planung/TODO.md) benannt, nicht still gelassen.
...
| [O39](#o39--das-ledger-ist-im-lan-für-alle-lesbar) LAN-Exposition | S | Die Exposition ist in [BETRIEB.md](../betrieb/BETRIEB.md) benannt; falls Read-Token gewählt: Test, dass `/api/v1/fills` ohne Token 401 antwortet, sobald `TANKAPP_READ_TOKEN` gesetzt ist, und unverändert offen bleibt, wenn nicht |
...
[Todo](../planung/TODO.md) bzw. in [LUECKEN.md](../planung/LUECKEN.md#ausstehender-betriebsnachweis).
...
| [O37](#o37--der-server-misst-sich-selbst-nicht) keine Metriken | S | Test: Antworten tragen `X-Process-Time`, `/api/v1/health` nennt Parse-Dauer und Publikationsgröße; Budget steht in [QUALITAET.md](../entwicklung/QUALITAET.md) |
| [O34](#o34--historie-und-archiv-haben-keine-aufbewahrungsregel) Aufbewahrung | S | Der InfluxDB-Cron rotiert (`find … -mtime +N -delete` im Befehl); [BETRIEB.md](../betrieb/BETRIEB.md) nennt die Archiv-Entscheidung mit Begründung |
...
[Todo](../planung/TODO.md), in [LUECKEN.md](../planung/LUECKEN.md#ausstehender-betriebsnachweis)
...
[QUALITAET.md](../entwicklung/QUALITAET.md) sind **nicht** neu gefahren (Batch 7 ändert keine
...
Form-Modell je Station stehen begründet im [Todo](../planung/TODO.md) und in
[LUECKEN.md](../planung/LUECKEN.md#ausstehender-betriebsnachweis). Alle vier Checks sind
...
| `demo_data.py` schreibt über `write_json`, also `jq -e . current.json` grün | erfüllt — der Demo-Stapel nutzt `write_split_publication` (dieselbe Funktion wie die Produktion, `json_safe`/`allow_nan=False`); vorher standen gemessen 51 480 `NaN`-Token in der Datei, an der [STATIONEN-TAUSCH.md](../betrieb/STATIONEN-TAUSCH.md) die `jq`-Prüfung nachvollziehbar macht. Jede geschriebene Datei besteht die strenge JSON-Prüfung |
