...
> Stand: 14.09.2026 · App-Version 0.31.0 · Branch `arena/01a09e7a-tankapp` (PR #112).
...
- [ ] **1.1** `read_series(poll_dir, uid, fuel)` ergänzen: liest dieselben
...
- [ ] **1.2** Endpunkt `GET /api/v1/series?station=<uuid>&fuel=<fuel>`
...
- [ ] **1.3** Snapshot-TTL-Cache (5 s) für `Context.snapshot()`: thread-sicher
...
- [ ] **1.4** `VERSION = "2.0"` → `"3.0"` anheben (Template-Auto-Update über
...
- [ ] **2.1** `_TEMPLATE_HEAD` + `_TEMPLATE_REST` durch das Mockup-Design
...
- [ ] **2.2** Vorher wissen: Die Tests prüfen das Template nur über den
...
- [ ] **2.3** Selbst-Check am Mockup-vs-Produkt-Delta: Verdict-Logik kommt
...
- [ ] **3.1** `tests/test_rp2_fallback.py` ergänzen (Fixtures existieren
...
- [ ] **3.2** Kompletter CI-Spiegel lokal grün, bevor gepusht wird
...
- [ ] **6.1** PR #112 umbenennen (Titel ohne „zur Abstimmung"), Body um
  „umgesetzt" ergänzen; diese Checkliste abhaken lassen.
