  ...
  return (
    <section aria-labelledby="system-title">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-mono text-slate-200">
                      {(collector ?? h?.collector)?.age_minutes != null ? `${deNumber((collector ?? h?.collector)?.age_minutes ?? 0, 0)} Min.` : "—"}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-mono text-slate-200">
                      {(collector ?? h?.collector)?.tmpfs_used_bytes != null
                        ? `${euro(Number((collector ?? h?.collector)?.tmpfs_used_bytes) / 1024 / 1024, 1)} MiB`
                        : "—"}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs">
      ...
      <div className={`${panel} mb-4 p-5 sm:p-6`}>
        ...
        <div className="mb-4 grid gap-3 sm:grid-cols-3">
          <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-3">
            ...
            <p className="mt-1 text-lg font-bold text-white">{coverage.stationCount}</p>
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-3">
            ...
            <p className="mt-1 text-lg font-bold text-emerald-300">{coverage.freshCount}</p>
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-3">
            ...
            <p className="mt-1 font-mono text-sm font-bold text-slate-100">{coverage.coverageWindow ? `${coverage.coverageWindow} Uhr` : "—"}</p>
            <p className="mt-1 text-[11px] text-slate-500">
              {coverage.coverageReference != null
                ? `Bestwert ${Math.round(coverage.coverageReference * 100)} % · Schwelle ${coverage.coverageThreshold != null ? Math.round(coverage.coverageThreshold * 100) : "—"} %`
            ...
            <>
              <div className="grid gap-3 text-xs sm:grid-cols-3">
                <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-3">
                  ...
                  <p className="mt-1 text-lg font-bold text-rose-300">{selection.data.dead_count ?? 0}</p>
                ...
                <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-3">
                  ...
                  <p className="mt-1 text-lg font-bold text-amber-300">{selection.data.price_twin_count ?? 0}</p>
                  <p className="mt-1 text-[11px] leading-relaxed text-slate-500">
            ...
            <Metric
              ...
              value={
                statsSummaryRes.data?.quality_metrics.top3_hit_rate != null ? (
                  <span className="font-mono text-emerald-300">{`${Math.round(statsSummaryRes.data.quality_metrics.top3_hit_rate * 100)} %`}</span>
                ) : (
      ...
      <div className={`${panel} p-5 sm:p-6`}>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-4">
            <p className="text-xs font-semibold text-slate-200">Belege &amp; Version</p>
            <p className="mt-1 text-[11px] leading-relaxed text-slate-500">
              Alle Belege als CSV — für die Steuer oder den eigenen Notizzettel. Version steht im Footer, Commit im Health-Endpunkt.
            </p>
        ...
        <div className="mt-6">
          ...
          <p className="mb-3 text-[11px] text-slate-500">
            Dieselben Endpunkte, die diese GUI nutzt — live abgerufen, ohne Poll auszulösen. Aktuelle Endpunkte: decide, episodes, fills, stats/summary.
          </p>
      ...
      <Level1Sheet
        ...
        sentences={systemExplanationLaeufe(h?.jobs as any).sentences}
        source={systemExplanationLaeufe(h?.jobs as any).source}
        labHint={systemExplanationLaeufe(h?.jobs as any).labHint}
        onDeepen={() => setSheet(null)}
        onClose={() => setSheet(null)}
      />
      <Level1Sheet
