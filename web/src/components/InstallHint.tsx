...
export function InstallHint({ onNote }: { onNote?: (message: string) => void }) {
  ...
  return (
    <section
      ...
      className="mb-6 flex flex-wrap items-center gap-3 rounded-xl border border-emerald-500/25 bg-emerald-500/10 p-4 text-sm text-emerald-100"
    >
      {promptEvent ? (
        <Download
          size={18}
      ...
      <button
        type="button"
        onClick={dismiss}
        className="flex items-center gap-1.5 rounded-xl border border-emerald-500/30 px-3 py-2 text-xs font-semibold text-emerald-200 hover:bg-emerald-500/10"
      >
