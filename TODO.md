...
| `multiprocessing.get_start_method()` | **`forkserver` auf Stand 0.25.1** | B19-Befund auf der Zielhardware (Python 3.14, gh-84559): Initargs wurden je Worker gepickelt, zwei Pools je Kraftstoff. **Seit 0.26.0:** explizit `fork`, ein Pool je Kraftstoff; Nachmessung ausstehend. |
...
Bis 0.25.1 nutzte `app/model_jobs.py::resolve_workers` `os.cpu_count()`, das
CPU-Affinität und cgroup-Quota ignoriert. Seit 0.26.0 kombiniert die Automatik
`os.process_cpu_count()`/Affinität mit `cpu.max` (B23). Auf dieser Hardware war
der alte Fehler ohne Wirkung, weil Host
...
| **4 — Feinschliff** ✅ (Code; NAS-Nachmessung ⏳) | **B19** + **B20** (Punkte 2, 4, 5, 6, 7) + **B23** + B11-Dokuwerte | ~1,5–2 Tage | ~28 s je Lauf war die Schätzung; Messwert nach Deploy offen | **Umgesetzt in 0.26.0:** ein expliziter `fork`-Pool je Kraftstoff statt zwei `forkserver`-Pools, schlanke 6-Spalten-Initargs, leere Backtest-Horizonte ohne `predict`, kleine Punktpayloads, `as_completed` bei stabiler Publikationsreihenfolge, unmittelbarer serieller Callback, monotoner Fortschritt 35–95 % im Fit und Worker-Automatik aus Prozesssicht + cgroup-Quota. **Ehrlich offen:** Laufdauer/Peak nach 0.26.0 auf dem NAS; Cache-Schema 2 erzwingt beim ersten Lauf einen Kaltlauf, `ops/nas/b11-cold-run.sh` liefert den Beleg. Punkt B20(3) (drei Horizonte in einen Task legen) war ausdrücklich nicht Teil dieses Batches. |
...
5. **Laufzeit-Bündel in sechs Batchen** — Batch 1 **B15/B16** ist mit 0.20.0
   umgesetzt (bitgleich, Tests vorhanden), Batch 2
   **B18+B24** mit 0.21.0 (1 Lauf/Tag statt 24, Abbrüche sichtbar), Batch 3
   **B17** mit 0.22.0 (Backtest je lokalem Endtag gecacht, Ausweis
   `backtest_cached`/`backtest_computed_at`), Batch 4 **B19/B20/B23** mit
   0.26.0 (ein `fork`-Pool, weniger Transfer, ehrlicher Fortschritt, robuste
   Worker-Automatik; NAS-Nachmessung noch offen). B22 bleibt nur eine
   Produktentscheidung. Batch 0 ist bis auf die
   B11-Messung auf der Zielhardware abgeräumt (Protokoll seit 0.25.1 für den
   Kaltlauf, Sammler in `ops/nas/measure-phase-b.sh`): **B21** war mit 0.25.0 ein
   echter Fehler — das Coverage-Gate der Selektion maß gegen das volle
   24-h-Raster und erreichte nie 85 % (77,5 % Maximalwert bei lückenlosem
   06–24-Polling, 4,8 % im Archiv-Betrieb), deshalb „e10: 0 Stationen“ und
   ein leeres „Meine Stationen“. Gate misst jetzt im Polling-Fenster und
   relativ zum Stadt-Bestwert; der Lock-Teil von B11 (5 s + `store_locked`
   als 503) ist mit umgesetzt. Plan mit Aufwand, Wirkung und Begründung je Batch
   steht im Abschnitt
