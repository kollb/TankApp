# TankApp — ToDo (Stand 13.09.2026, App-Version 0.25.2)
...
| `free -h` auf dem Host | 15 Gi gesamt, 11 Gi belegt, **4,2 Gi verfügbar**, Swap 0 | 4 Worker × ~150 MB ≈ 0,6 GB passen — Abgleich kalt am 13.09.2026 (Container-Peak 1,0 GiB, Host min 4,1 GiB verfügbar, shm 1 MiB, CPUS max 381 %; B11 erledigt, siehe unten). |
...
**B11 — Ressourcen-Abgleich, strenger Kaltlauf 13.09.2026** (NAS `Tower`,
Container `tankapp-web-app-1`, Sammler `ops/nas/b11-cold-run.sh`, 25 Stichproben
à 5 s, 17:36:21–17:39:16 local / 15:36:22–15:38:57 UTC). Cache vorher gelöscht
und verifiziert (19 Dateien weg, Mount `/mnt/user/appdata/TankApp/data/runtime`).
Job-Log: „Backtest: 0 aus Tages-Cache, 19 neu gerechnet“ — **KALT, Beleg gültig.**
20 Stationen, e10, 77 Tasks nach `retotal`, Endzustand `partial (some_models_unavailable)`,
19 Prognosen, 1 Fehler, Dauer **2,6 min**.

| Größe | Wert | Folge |
|---|---|---|
| Python-Prozesse | max **2** gezählt | Sammler matchte `cmdline` gegen `python*` — trifft Server + `python -m app.worker`; Forkserver-Kinder starten als `/usr/local/bin/python…` und fielen durch. Gegenprobe: CPUS max 381 % ≈ 4 Worker. Der Sammler zählt seit 0.25.2 cmdline **und** `/proc/pid/comm`. |
| Container-Speicher | max **1031 MiB (1,0 GiB)**, MEM % max **6,6** | Peak am Ende von Phase B (17:38:47); Leerlauf 102–117 MiB; nach Veröffentlichen 414 → 109 MiB. Passt zur Schätzung 4 × ~150 MB + Master. |
| Host verfügbar | min **4212 MiB (4,1 GiB)**, Swap 0 | Weit über der kritischen Marke ~500 MiB. |
| CPUS | max **381 %** (Phase B 248–381 %) | Pool lief mit ~4 Workern (B23-Gegenprobe). |
| `/dev/shm` | max **1 MiB** von 256 MiB | `shm_size: 256m` bleibt, keine Änderung in `compose.yml`. |

Der erste Messlauf desselben Tags (14:26–14:29, 1029 MiB / 370 %) war **kein**
Kaltlauf („9 aus Tages-Cache, 10 neu gerechnet“, 2,1 min) — am Peak ändert das
nichts, der je-Station-Aufwand der Worker ist cache-unabhängig. Die 2,6 min kalt
sind die ausstehende NAS-Gegenmessung für **B15/B16** (Erwartung ~2,3 min nach
10,3 min am 12.09.) und der Kaltstart des Tages-Caches aus Batch 1/3.
...
| **3 — Architektur** ✅ | **B17** (Tages-Cache) | ~2–3 Tage | untertägig **~1 min** (nur `wide72`/`wide168` bleiben) | **Umgesetzt in 0.22.0** (`app/backtest_cache.py`: Fingerabdruck = Config + Inhalts-Hash der gesamten Reihe bis Endtag + Schema-/Bibliotheksversionen; Ausweis `backtest_cached`/`backtest_computed_at`; Tests in `tests/test_backtest_cache.py`). Nebenbefund: die +3-d/+7-d-Fenster wurden bisher gegen den laufenden Tag bewertet — `run_backtest` schneidet jetzt hart am Ende ab. B20 Punkt (1) — toter `fit` im backtest-Task — ist damit erledigt. **NAS-Gegenmessung 13.09.2026 (Job-Log, drei Läufe 07:52/10:15/10:50 UTC):** **1,4–1,7 min** je Lauf statt 10,3 min am 12.09. — Backtest komplett aus dem Tages-Cache („19 aus Tages-Cache, 0 neu gerechnet“), Phase B damit nur noch ~50 s. Kaltstart des Caches gemessen 13.09.2026 17:36 local: **2,6 min** („0 aus Tages-Cache, 19 neu gerechnet“) gegenüber 1,4–1,7 min warm. |
| **4 — Feinschliff** | **B19** + **B20** (Punkte 2, 4, 5, 6, 7) + **B23** | ~1,5–2 Tage | ~28 s je Lauf, ehrlicher Fortschritt, robuste Worker-Zahl | Lohnt erst, wenn die großen Anteile weg sind; die B19-`initargs` hängen an derselben Datenstruktur wie B17 |
...
| Version | Punkte |
|---|---|
| 0.25.2 (13.09.2026) | **B11** Ressourcen-Abgleich auf der Zielhardware abgeschlossen: strenger Kaltlauf (`ops/nas/b11-cold-run.sh`, „Backtest: 0 aus Tages-Cache, 19 neu gerechnet“, 25 Stichproben) — Container-Peak **1,0 GiB**, Host min **4,1 GiB** verfügbar, `/dev/shm` **1 MiB** von 256 MiB, CPUS max **381 %**, Dauer **2,6 min**. `shm_size: 256m` bleibt. Sammler zählt Python-Prozesse jetzt über cmdline und `/proc/pid/comm` (dieser Lauf: max 2, weil `python*` Forkserver-Kinder mit vollem Pfad verfehlte; Gegenprobe ist die CPU). Zugleich Kaltstart-Gegenmessung für B15/B16: 10,3 → 2,6 min. |
...
5. **Laufzeit-Bündel in sechs Batchen** — Batch 1 **B15/B16** ist mit 0.20.0
   umgesetzt (bitgleich, Tests vorhanden; NAS kalt 10,3 → **2,6 min**, warm
   1,4–1,7 min), Batch 2 **B18+B24** mit 0.21.0 (1 Lauf/Tag statt 24,
   Abbrüche sichtbar), Batch 3 **B17** mit 0.22.0 (Backtest je lokalem Endtag
   gecacht, Ausweis `backtest_cached`/`backtest_computed_at`). Weiter:
   B19/B20/B23 (Feinschliff), B22 nur als Produktentscheidung. Batch 0 ist
   vollständig abgeräumt: **B21** war mit 0.25.0 ein echter Fehler — das
   Coverage-Gate der Selektion maß gegen das volle 24-h-Raster und erreichte
   nie 85 % (77,5 % Maximalwert bei lückenlosem 06–24-Polling, 4,8 % im
   Archiv-Betrieb), deshalb „e10: 0 Stationen“ und ein leeres „Meine
   Stationen“. Gate misst jetzt im Polling-Fenster und relativ zum
   Stadt-Bestwert; der Lock-Teil von B11 (5 s + `store_locked` als 503) ist
   mit 0.25.0 umgesetzt, die Messung auf der Zielhardware mit 0.25.2
   (Kaltlauf 1,0 GiB / 4,1 GiB / shm 1 MiB / 381 %). Plan mit Aufwand,
