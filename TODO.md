# TankApp — ToDo (Stand 19.09.2026, App-Version 0.58.0)
...
*Keine offenen Punkte.* **B25** (zweites automatisches Backup-Ziel) und
**B22** (`bootstrap_samples`, Nacht-Raster) sind am 18.09.2026 als *nicht
nötig* geschlossen — Begründung unten unter
[Geschlossen als nicht nötig](#geschlossen-als-nicht-nötig-18092026).

---

## C. GUI / UX

*Keine offenen Punkte.* **C13** (mobile Kopfzeile verdichten) und **C12**
(Desktop-Zweispalter) sind am 18.09.2026 als *nicht nötig* geschlossen —
Begründung unten unter
[Geschlossen als nicht nötig](#geschlossen-als-nicht-nötig-18092026).
...
Der Text-Befund
[docs/TEXT-BEFUND.md](docs/archiv/TEXT-BEFUND-2026-09-15.md) (T1–T13) war die Vorlage für 0.39.0.
...
- **H8 (P0, bis 30.09.2026)** — **Die Gate-Zähler gegen den Bruch immun
  ...
  **DoD:** `mase_scale` wird am Regime-Kalender normalisiert (oder das Gate
  ...
  trägt im Ledger und im Bericht ein Regime-Kennzeichen. *Teilstück seit
  0.56.0 (B0):* der Backtest-Bericht trägt `regime_breaks_in_window` und
  markiert Folds/Zeilen über eine Kante (`regime_break_spanned`), der
  Kalender kommt aus `TANKAPP_REGIMES`; `mase_scale`, Gates, Alarme und das
  Ledger-Kennzeichen sind davon unberührt und bleiben dieser Punkt.
...
| 0.56.0 (19.09.2026) | **B0 — Messgrundlagen des [UX/Mathe-Befunds](docs/BEFUND-UX-MATH-2026-09-19.md#b0--messgrundlagen-unsichtbar-bitgleich) (unsichtbar, bitgleich):** Zähler im Modell-Artefakt (`ar_shrink_events`, `ar_state_reset`, `ar_detail` mit Rückfallgrund, `ensemble.weight_spread` je 288-Slot-Block), `pava_pool_stats` als `predict`-Diagnose (Abweichung vom Befund: PAVA läuft erst in der Prognose, deshalb kein Artefakt-Feld), PIT-Paare je Zeile und Horizont in `predictions.csv.gz` (`pit`, `horizon_hours`) mit Histogramm je Station im Bericht (`all`/`break_free`), Regime-**Kalender** statt bloßem Marker (`Config.regimes`: Zeit, Art, Sorte, Betrag, Status, Quelle — `TANKAPP_REGIMES`, Default die vier bekannten Termine, kaputter Kalender bricht den Lauf mit Grund ab), `regime_breaks_in_window` mit Politik *markiert, nicht ausgeschlossen*, `ar_shrink` je Backtest, `--regime-break`/`--kind`/`--shared-draws` in der CLI, Backtest-Cache Schema 3, Messfelder je Station in der Veröffentlichung. **Bitgleich bewiesen:** `tests/test_b0_invariance.py` gegen eine Fixture aus dem Code vor der Änderung. **Dabei gefunden, nicht geändert:** Der Backtest misst `harmonic_ar2` mit unabhängiger Ziehung, veröffentlicht wird das Ensemble mit gemeinsamer Ziehung — jetzt benannt (`backtest_model_kind` neben `model_kind`), Umschalten ist B3 ([LUECKEN.md](docs/LUECKEN.md#bewusst-offen-backlog-mit-grund)). **Nicht gelaufen:** Referenzmessung PICP/Brier/MASE je Station (keine NAS-Daten hier; Rezept in [ENGINE.md](docs/ENGINE.md#messgrundlagen-b0-seit-0560), Brier gibt es nur global aus dem Ledger). Nachweis: 1201 Pytest (neu `test_b0_invariance.py` 10, `test_b0_counters.py` 16, `test_b0_pit_regime.py` 21, `test_b0_app.py` 11), 1230 Vitest, Ruff, Build; beide Playwright-Suiten in dieser Umgebung grün über den `@sparticuz/chromium`-Weg aus QUALITAET.md (Alltag 38/38 gegen den leeren Server, Demo + Mobil 45 grün / 12 skipped). |
| 0.54.0 (18.09.2026) | **Batch 8 des [Optimierungs-Befunds](docs/archiv/OPTIMIERUNGS-BEFUND-2026-09-18.md#batch-8--p3--schliff) abgeschlossen — und damit der ganze Befund:** **O32** Die Belegmaske zeigt den Live-Preis der gewählten Station **mit Alter** („Jetzt an der Station: 1,719 €/L, gemeldet vor 3 Minuten.“) und markiert eine Abweichung ab 1,0 ct/L mit Richtung — zwischen Empfehlung und Erfassung liegen Minuten bis Stunden (Offline-Queue), vorher stand dort eine vorbefüllte Zahl ohne Vergleich. Das Alter kommt aus `observed_at`, ersatzweise `age_minutes`; fehlt beides, bleibt es ungenannt statt geschätzt, ohne frischen Preis steht nichts da, und gebucht wird weiterhin das Getippte (`fillPriceHint()` in `web/src/fills.ts`, kein Entscheiden in der View). **O40** Die Diagramm-Textalternative nennt Werte statt Reihennamen: `web/src/chartAlt.ts` baut aus denselben Punkten, die gezeichnet werden, Anfang/Ende/Richtung, Tief und Hoch (nur wenn sie nicht die Endpunkte sind), Bandspanne, Histogramm-Mitte, beide Balken-Ausschläge mit Stationsnamen und die mittlere Abweichung von der Diagonalen — alle Zahlen über die Formatter, ohne Punkte der ehrliche Kurztext. Dazu ein eigenes `aria-label` je Diagramm (vorher hießen alle „Diagramm“, im Labor liegen vier in einer Ansicht). Nebenbefund: `LineChart` hatte `xFmt` als Default-Parameter — für den Tooltip richtig, für die Beschreibung falsch (die Stundenachse 0–24 wäre als Datum vorgelesen worden). **O28 und O41 lagen seit PR #154 auf `main`, ohne dass Version, CHANGELOG oder Befund es sagten** — mit dieser Zeile ist der Ledger-Drift geradegezogen. Nachweis: 1129 Pytest, 1180 Vitest (neu `chartAlt.test.ts` 20, O40-Ratchet in `a11y.test.ts` 5, O32 in `fills.test.ts` 8 und `views/Ich.test.tsx` 3), Ruff, Build; Playwright in dieser Umgebung nicht installierbar. |
| 0.53.0 (18.09.2026) | **Ein Graph weniger, „Jetzt“ mobil verdichtet (Nutzerurteile 18.09.2026):** Der Mini-Verlauf der Stationszeile ist entfallen (96 × 24 px ohne Achse, Zeitbezug und y-Skala — „niemand kann was mit dem Graphen anfangen“); `stripSparkline` samt Test ist entfernt, der Verlauf bleibt als Knopf je Zeile im Stations-Detail, ein Ratchet in `views/Stationen.test.tsx` verhindert die Rückkehr. „Zu lang auf mobil“: Der Einstieg maß bei 390 × 844 **2 530 px** gegen **1 223 px** des Entwurfs (`ui-neuentwurf-mockup`). Verdichtet sind die drei Fakten (3er-Reihe, Tank-Fakt zweite Zeile; 409 → 190 px), „Heute im Blick“ (Kennzahlen als Zeilenliste statt Karten; 706 → 504 px), die Stationszeilen (Name erste Zeile, Preis/Knöpfe zweite — vorher „Demo-T…“) und die Marke in der Rangliste (erst ab `sm`). Gesamt **2 118 px**. Kopfzeile (197 px, vier Steuerzeilen) bewusst offen mit Grund (C13/LUECKEN). Neu: Mobil-Zusage im Browser (`web/e2e/mobile.spec.ts`). Dazu die Doku-Abgrenzung „`reportAllChanges`/`startTime` ist die DevTools-Kopie von web-vitals, kein App-Frame“ (BETRIEB.md). Nachweis: 1116 Pytest, **1144 Vitest**, Ruff, Build; **beide Playwright-Suiten erstmals in dieser Umgebung grün** (Alltag 38/38 gegen den leeren Server, Demo + Mobil 25 grün / 11 skipped). |
| 0.51.0 (18.09.2026) | **B30 — 12-Uhr-Bodenkante (Schritt 3 des [12-Uhr-Befunds](docs/archiv/BEFUND-12-UHR-REGEL-2026-09-18.md)):** Beobachtungs-Panels, Selektion, Modell-Fit und beide Offline-Werkzeuge zählen nur Beobachtungen **ab** `price_law_local` (Heatmap, Selektion, `engine/models.py::fit`, `engine/station_comparison.py`, `analysis/station_selection.py`); Payload, GUI und Publikations-Index nennen Kante und ausgeblendete Punkte, der Modell-Lauf misst den Anteil Vorsgesetzliches (`law_quality`). Kein Hardcode: Die Kante liest `price_law_local`, `TANKAPP_LAW_FLOOR=0` bzw. `--ignore-law-floor` stellen den Mischbestand für Gegenmessungen wieder her. Konzept: [UMSETZUNG-B30-12-UHR-BODENKANTE.md](docs/archiv/UMSETZUNG-B30-12-UHR-BODENKANTE-2026-09-18.md). Dabei nachgerechnet: Die Prämisse war veraltet — alle Muster-Fenster (42/84/120 Tage) beginnen heute hinter dem 01.04.2026, die Kante ist Garantie statt Reparatur (Befund §4.1). **Offen:** der DoD-Backtest „ohne Qualitätsverlust“ braucht echte NAS-Daten über beide Rechtslagen — Runbook liegt, gelaufen ist er nicht. |
| 0.50.0 (17.09.2026) | **Batch 6 des [Optimierungs-Befunds](docs/archiv/OPTIMIERUNGS-BEFUND-2026-09-18.md#batch-6--p2--anzeige-und-alltag) — Anzeige und Alltag:** O21 (eine Quelle für den Score, `tests/fixtures/score_parity.json` nagelt Python und TypeScript auf dieselben Eingaben; Tankmenge aus dem Profil bis in `SelectionConfig.tank_volume`, `liters`/`eps` fahren je Score-Block mit), O19 (Ersparnis gegen dieselbe Referenz wie `p_lohnt`/`ref_nowcast`, Spanne bleibt Spanne), O20 (feste Farbskala über 25/75-Perzentil statt Tages-Min/Max, je Stunde das Minimum statt der letzten Meldung), O30 (`saved_net_eur` nach Umweg mit derselben Formel wie die Entscheidung, brutto und netto benannt), O18 (ε-Scan rechnet auf den Backtest-Zeilen nach, Rang-Streuung/Strukturbruch lesen `rank_std`/`break_flag`/`break_stat`, Modellvergleich nennt den Dauerzustand statt μ = 1,5 ct zu erfinden; CUSUM-Schwelle war 3.0 publiziert bei h = 2.0 — jetzt `engine.selection.CUSUM_THRESHOLD`), O31 (Wochen-Rückblick über den O29-Kanal, einmal je ISO-Woche, nie in der Ruhezeit, nur vorhandene Größen, `mode="public"` ohne Station und Preis), O39 (LAN-Exposition in BETRIEB.md benannt, `TANKAPP_READ_TOKEN` schützt die Ledger-Routen, leer = offen). Nachweis: 1045 Pytest, 1129 Vitest, Ruff, Build; Playwright in dieser Umgebung nicht installierbar. |
| 0.49.1 (17.09.2026) | **Zwei Abstürze aus dem Produktionsbetrieb vom 17.09.2026 (Abend), O44 des [Optimierungs-Befunds](docs/archiv/OPTIMIERUNGS-BEFUND-2026-09-18.md#o44--zwei-abstürze-aus-dem-produktionsbetrieb-fremde-antwortformen-brechen-die-seite):** (1) `app/pside.py::_supported` hielt `None` (aus `null` in der publizierten Draw-Datei) für einen Messwert und warf beim Sortieren `TypeError: '<' not supported between instances of 'float' and 'NoneType'` — `decide` scheiterte, die GUI zeigte nur `decide_failed`; jetzt zählt `None` wie `NaN` („keine Aussage“), und `app/data.py` meldet jede unerwartete Decide-Ursache bereinigt als `detail` bis in die Oberfläche. (2) Der Fehlerpayload ließ `web/src/views/Stationen.tsx` an `decide.alternatives_nearby.find` abstürzen (Bereich „Stationen“ zusätzlich zur fehlenden Empfehlung); der Vergleich liest jetzt `?? []`. (3) Im Fallback-Modus (`<RP2-IP>:8000`) brach die Seite mit `Cannot read properties of undefined (reading 'includes')` weiß ab: `rp2/fallback_gui.py::_api_stations` lieferte kein `cities`, die SPA liest es zur Ortswahl; die Antwort trägt jetzt `cities`, je Zeile `observed_at` und `calibrated/decision_ready: false`, `_api_series` akzeptiert `station_id`; die App filtert fremde Payloads über `usableStations()` und meldet „Antwort kommt vom Pi-Fallback“. Nachweis: 982 Pytest (neu `tests/test_o44_null_draws.py`, +3 in `test_rp2_fallback.py`), 1097 Vitest (neu `pi-fallback.test.tsx`, +2 in `data.test.ts`), Ruff, Build. |
...
| 0.20.0 (12.09.2026) | **B15** Bootstrap-Pfade vor der 12-Uhr-Projektion je Segment dedupliziert (`engine/models.py::project_paths`, `np.unique(…, axis=0, return_inverse=True)`, NaN als Stellvertreter kodiert damit identische Ziehungen mit gleichem NaN-Muster zusammenfallen), **B16** `fit()` von String- und Aggregator-Overhead befreit (a+c Residuen-Tagesblöcke als (Tag, Slot)-Index-Zuweisung mit `pd.factorize`-Tagesschlüssel, b Feiertagsmaske über `searchsorted` auf int64-Tageswerte in `engine/holidays.py`, d Naiv-Profil über stabilen Sortierindex + `searchsorted`, e `law_rise_outside_noon` vektorisiert — alles bitgleich; die Huber-Normalgleichungen bleiben aus). Bitgleichheits-Tests in `tests/test_models.py` (`project_paths` vs. skalare Referenz, `_residual_blocks` vs. `pivot_table`, `_naive_profile` vs. `groupby`, `holiday_f-Normalgleichungen bleiben aus). Bitgleichheits-Tests in `tests/test_models.py` (`project_paths` vs. skalare Referenz, `_residual_blocks` vs. `pivot_table`, `_naive_profile` vs. `groupby`, `holiday_flags` vs. Set-Mitgliedschaft). NAS-Gegenmessung aus dem Job-Log steht noch aus |
...
## Geschlossen als nicht nötig (18.09.2026)

Entscheidung des Betreibers nach Abschluss des Optimierungs-Befunds (0.54.0):
Die vier verbliebenen Arbeitspunkte und die offenen Betriebsaufgaben werden
**nicht** umgesetzt. Sie stehen hier mit Grund, statt als Karteileichen in den
Tabellen oben zu altern — wer einen davon doch will, findet hier den Stand und
die ursprüngliche Definition of Done im [CHANGELOG](CHANGELOG.md) und in
[docs/LUECKEN.md](docs/LUECKEN.md).

| # | Thema | Warum geschlossen |
|---|---|---|
| B25 | Zweites automatisches Backup-Ziel | Der Schutz, der zählt, steht: Die App meldet die Backup-Alterung selbst (`backup_stale` ab 36 h, dazu `no_backup`/`directory_missing`), `ops/nas/backup.sh` behält 14 Tages- plus 6 Monatsstände, und für den Gerätetotalausfall gibt es die **manuelle** Zweitkopie als dokumentierte Entscheidung ([BETRIEB.md](docs/BETRIEB.md#nas-laufzeitdaten-runtime-backup)). Ein zweites automatisches Ziel bräuchte ein zweites Gerät, das es im Haushalt nicht gibt — Automatik gegen eine Hardware, die nicht da ist, wäre ein Alarm ohne Adressat. |
| B22 | `bootstrap_samples` 2000→500, Nacht-Raster | Reine Laufzeit-Optimierung an einem Lauf, der niemanden warten lässt: Der Modell-Lauf läuft nachts auf dem NAS, seine Dauer ist seit B15–B17 kein Problem mehr. Der Hebel würde dafür MASE/PICP/MPIW verschieben — schlechtere Zahlen gegen eine Ersparnis, die kein Mensch bemerkt. |
| C13 | Mobile Kopfzeile verdichten | 0.53.0 hat die Verdichtung gebracht, die den Anlass hatte (2 530 px → 2 118 px bei 390 px, Zusage in `web/e2e/mobile.spec.ts`). Die restlichen 197 px sind vier **Steuerzeilen** — Stadt, Kraftstoff, Profil, Knöpfe. Sie weiter zu verdichten hieße, Steuerungen hinter ein Blattmenü zu räumen: ein Griff mehr im Alltag für einen Bildschirm, der schon passt. |
| C12 | Desktop-Zweispalter je Bereich | Der Entwurf ([UI-NEUENTWURF](docs/UI-NEUENTWURF.md) §13) sieht ab ~1280 px zwei Spalten vor; die App wird aber am Handy benutzt, und einspaltig ist am Desktop nicht falsch, nur breit. Zwei Spalten je Bereich sind ein Umbau aller Ansichten samt Browser-Zusagen — Aufwand ohne Beschwerde. |

**Ebenfalls geschlossen — Betriebsaufgaben, die keine Code-Tasks waren:**

- **B30-DoD-Backtest „ohne Qualitätsverlust“ auf echten NAS-Daten über beide
  Rechtslagen.** Die Bodenkante ist seit 0.51.0 umgesetzt und geprüft; der
  Gegenbeweis über die alte Rechtslage bräuchte einen Mischbestand, den es
  produktiv nicht mehr gibt. `TANKAPP_LAW_FLOOR=0` bleibt als Schalter für
  Gegenmessungen, falls die Frage doch einmal auftaucht.
- **Nachher-Dauer des Modell-Laufs nach dem 0.26.0-Deploy.** Die Messung
  sollte belegen, was seither im Alltag sichtbar ist: Der Lauf stört nicht.
  Eine Zahl nachzureichen, die keine Entscheidung mehr trägt, ist Arbeit ohne
  Adressat.
- **Lighthouse-Performance-Budget von Warnung auf Fehler ≥ 0,90 nachziehen.**
  Gemessen wurden 0,97–0,98, das M4-Ziel ist erfüllt und die drei anderen
  Kategorien stehen bereits auf `error`. Das Budget bleibt bewusst Warnung:
  Der Lauf misst einen Demo-Stack auf geteilten CI-Läufern, ein hartes Gate
  auf 0,90 würde irgendwann an der Läufer-Auslastung scheitern, nicht an der
  App ([QUALITAET.md](docs/QUALITAET.md#budgets)).
- **`docs/UMSETZUNG-FALLBACK-GUI-V2.md` §5.2 und §6.2** (Abnahme auf dem
  Pi/RP2, Archivierung nach dem Merge). §6.2 ist mit dem Aufräumen vom
  18.09.2026 erledigt — das Dokument liegt jetzt im Archiv. §5.2 verlangt die
  echte Hardware; der Fallback läuft dort seit Monaten im Betrieb, eine
  nachgereichte Häkchen-Abnahme ändert daran nichts.
...
3. **Kein P0 mehr offen** — der Heatmap-P0 vom 12.09. ist mit 0.14.0
   geschlossen, B2 (Schema-Version) seit 0.13.0, und der
   [Optimierungs-Befund](docs/archiv/OPTIMIERUNGS-BEFUND-2026-09-18.md#10-batches-priorität-und-check)
   ist mit Batch 8 (0.54.0) vollständig abgearbeitet. Der Store-Feldsprung von
   O1 (Schema 4, `clock_hour_source`) bleibt das Muster für künftige
   Datenänderungen: Migrationsfunktion nach `app/feedback.py::_STORE_MIGRATIONS`
   plus `FEEDBACK_SCHEMA_VERSION` anheben — Altbestände kennzeichnen, nie still
   umschreiben.
4. **Was auf echte Live-Daten wartet** (ACI, M3-Zweitmodell, w(h)-Rückkopplung)
   steht weiter mit Status in
   [docs/LUECKEN.md](docs/LUECKEN.md#bewusst-offen-backlog-mit-grund) — kein
   Arbeitspunkt, sondern eine Frage der Betriebszeit.
5. **Laufzeit-Bündel ist Code-seitig durch.** Batch 1 **B15/B16** 0.20.0,
   ...
   mit 0.26.1 geschlossen (Kaltlauf 2,6 min / 1,0 GiB). **B22 und die
   ausstehende Nachher-Messung von 0.26.0 sind am 18.09.2026 als nicht nötig
   geschlossen** ([oben](#geschlossen-als-nicht-nötig-18092026)): Der Lauf
   stört im Alltag niemanden, und der verbliebene Hebel würde Kennzahlen
   verschieben statt Wartezeit sparen. Protokoll der Messungen in
   [docs/BETRIEB.md](docs/BETRIEB.md#ressourcen-während-phase-b-messen-b11).
